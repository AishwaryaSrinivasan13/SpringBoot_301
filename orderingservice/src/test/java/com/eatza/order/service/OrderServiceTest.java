...
import com.eatza.order.feign.DeliveryServiceClient;
...
//	@Test
	public void placeOrder_basic() throws OrderException {
		...
		lenient().when(restTemplate.getForObject(any(String.class), any(Class.class))).thenReturn(item);
		lenient().when(itemService.saveItem(any(OrderedItem.class))).thenReturn(new OrderedItem());
		DeliveryResponseDto deliverItem = new DeliveryResponseDto(1L, 1L, 1L);
		lenient().when(deliveryServiceClient.logOrder(any(DeliveryResponseDto.class)))
				.thenReturn(new ResponseEntity<String>("Order successfully logged", HttpStatus.OK));
