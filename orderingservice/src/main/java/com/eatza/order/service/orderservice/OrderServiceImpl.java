	...
	public Order placeOrder(OrderRequestDto orderRequest) throws OrderException {
		...
		for (OrderedItemsDto itemDto : itemsDtoList) {
			...
			try {
				...
				DeliveryResponseDto deliverOrder = new DeliveryResponseDto(savedOrder.getId(),
						savedOrder.getRestaurantId(), savedOrder.getCustomerId());
//				System.out.println("ORDER\n" + deliverOrder);
//				ResponseEntity<String> deliver = this.deliveryServiceClient.logOrder(deliverOrder);
				Map<String, Object> headers = new HashMap<>();
				headers.put(KafkaHeaders.TOPIC, topicName);
				kafkaTemplate.send(new GenericMessage<DeliveryResponseDto>(deliverOrder, headers));

				savedOrder.setLoggedDeliverable("YES");
